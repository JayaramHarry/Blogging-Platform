...
const Home = ({searchTerm}) => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('http://blogging-platform-hm.glitch.me/src/server/posts');
  ...
  const handleDelete = async (postId) => {
    try {
      await fetch(`http://blogging-platform-hm.glitch.me/src/server/posts/${postId}`, {
        method: 'DELETE',
      });
...
// Reset editing mode
  ...
  const handleUpdate = async (postId, updatedPost) => {
    ...
    try {
      const response = await fetch(`http://blogging-platform-hm.glitch.me/src/server/posts/${postId}`, {
        method: 'PUT',
        headers: {
